...
The CLI is for humans and CI. Skills are for agents. Eleven `/straymark-*` slash-commands ship with the framework so Claude Code, Antigravity CLI, Copilot CLI and others can invoke Charter scaffolding, AILOG creation, audit cycles, status checks and security assessments without you having to dictate the steps.
