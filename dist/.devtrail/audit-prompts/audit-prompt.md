<!--
DevTrail audit prompt — unified template (v1).
...
`.devtrail/audits/<CHARTER-ID>/audit-prompt.md` — the contents are the same.)
...
will use whatever lives at `.devtrail/audit-prompts/audit-prompt.md` at
...
`audit/SKILL.md` skill mature pre-DevTrail in Sentinel, contributed via
...
DevTrail orquesta auditorías cross-modelo: típicamente otro auditor de una **familia de modelo distinta** está revisando el mismo Charter en paralelo. Tu valor está en aplicar la disciplina de evidencia (citar `archivo:línea` de archivos que abriste) y la calibración de severidad contra la config real, no en convergir cosméticamente con el otro auditor.
...
- En modo CLI auditor-side (skill `devtrail-audit-execute`): `.devtrail/audits/{{charter_id}}/report-<sluggified-model-id>.md` (la skill maneja el path automáticamente).
...
- **NO ignores** las carpetas de auditorías previas (típicamente `audit/` o `.devtrail/audits/`) — contienen análisis previos que NO debes auditar (ya fueron auditadas o son meta-evidencia del proceso, no código del proyecto).
