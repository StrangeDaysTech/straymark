...
- **Missing-target behavior** *(clarified in fw-4.16.1)*: each command treats a missing injection target differently. `straymark init` and `straymark update-framework` walk `dist-manifest.yml::injections:` end-to-end and **create the target file** (with the marker block) if it does not yet exist — there is no opt-out per target short of editing the manifest. `straymark repair` is narrower: it re-injects directives **only when `STRAYMARK.md` itself is missing** (the assumption is that a corrupted install drops `STRAYMARK.md` along with everything else). If a single injection target is missing in an otherwise-healthy install — e.g. an operator deleted `AGENTS.md` while keeping `STRAYMARK.md` — re-run `straymark update-framework` to restore it; `straymark repair` will skip the restore in that case.
