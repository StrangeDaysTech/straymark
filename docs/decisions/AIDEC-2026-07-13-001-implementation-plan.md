...
| **S3** | Skew visibility: `straymark status` shows `installed fw-X (pinned)` vs `dist/ in-dev fw-Y` | CLI `status` | ⏳ Fase 1 |
| **S4** | Agent directive: `/.straymark/` = governance-in-force; `/dist/.straymark/` = product-under-edit | `CLAUDE.md`/`AGENTS.md` (docs) | ⏳ Fase 1 |
| **S5** | Git strategy for the install: version artifacts; gitignore the pinned framework-file copies | at `init` time | ⏳ Fase 1 |
...
- **Fase 1 — Context safeguards.** S3 (status skew) · S4 (agent directive) · S5 (git strategy).
