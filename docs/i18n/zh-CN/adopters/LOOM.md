...
**意图平面（仅限 SpecKit 项目）。** 在状态覆盖层之外，Loom 0.7.0 增加了可切换的第二平面——工具栏上的 **状态 | 意图** 开关——将你的*设计意图*叠加到同一张地图上，由 [Baton](./BATON.md)（实验性）计算。你在 SpecKit 记忆（`.specify/memory/Arquitectura - X.md` / `Architecture - X.md`）中命名的组件会与模型和源文件匹配，并按结论上色：**已设计并已实现**、**已设计，未实现**（计划存在但没有代码与之匹配）、或**已实现，未设计**（代码存在但没有任何记忆文档命名过它）；覆盖层未涵盖的组件保持淡中性色。图例和组件详情徽章随平面切换，2D 与 3D 同样生效。没有 SpecKit 记忆的项目永远看不到这个开关。
