...
//! server stays read-only; writing files is the CLI's job (NFR4).
...
use straymark_core::architecture::{parse_model_str, ArchModel, Component, Layer};
use straymark_core::document::{detect_doc_type, discover_documents, DocType};
use straymark_core::drift::glob_match;

use super::{adr_mining, drawio};
use crate::utils;

/// Directories never scanned for components (build output, VCS, deps).
...
pub fn run(path: &str, force: bool, out: Option<&str>) -> Result<()> {
    ...
    let model_path = out_dir.join("model.yml");
    let drawio_path = out_dir.join("plan.drawio");
    ...
    let dirs = top_level_source_dirs(&root);
    if dirs.is_empty() {
        bail!(
    ...
    let enrich = enrich_from_adrs(&root, &mut model);
    ...
    utils::success(&format!(
        ...
        plural(model.components.len()),
        model.layers.len(),
        plural(model.layers.len()),
    ));
    utils::success(&format!("Wrote {}", drawio_path.display()));
    report_enrichment(&enrich);
    utils::info(&format!(
...
/// Does not require a `.straymark/` install (generate scans code).
...
fn top_level_source_dirs(root: &Path) -> Vec<String> {
...
/// Collect source files (paths relative to `root`), skipping excluded dirs.
...
fn enrich_from_adrs(root: &Path, model: &mut ArchModel) -> Enrichment {
    let mut e = Enrichment::default();
    let adrs = discover_adrs(root);
    e.adrs_scanned = adrs.len();

    // kebab(id) → component index, for matching mined names.
    ...
    for adr in &adrs {
        ...
        // Candidates that can name/relabel a component: C4 element ids/labels +
        // Affected-Components first cells.
        for el in &mined.elements {
            match match_component(&by_id, &model.components, &el.id, Some(&el.label)) {
                Some(idx) => {
        ...
        for row in &mined.affected {
            if match_component(&by_id, &model.components, &row.component, None).is_none() {
                unmatched.insert(row.component.clone());
            }
        }
        // Links from C4 relationships between two resolved components.
...
fn report_enrichment(e: &Enrichment) {
    if e.adrs_scanned == 0 {
        utils::info("No ADRs found — model seeded from codebase structure only.");
        return;
    }
    utils::info(&format!(
...
fn discover_adrs(root: &Path) -> Vec<PathBuf> {
...
fn render_model_yaml(model: &ArchModel) -> String {
